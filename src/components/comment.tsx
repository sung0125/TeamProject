...
import { createComment, deleteComment, getCommentsByPostId } from "@/actions/commentActions";
...
export default function CommentSection({ postId, postType }: CommentSectionProps) {
  ...
  const renderComment = (comment: Comment) => (
    <div key={comment._id} className={`border-b pb-4 ${comment.depth > 0 ? 'ml-8' : ''}`}>
      ...
      {/* 답글 작성 폼 */}
      {replyTo === comment._id && (
        <form onSubmit={(e) => handleCommentSubmit(e, comment._id)} className="mt-2 ml-8">
          <textarea
            value={newComment}
